...
if (missingMeta.length > 0)
    log.error(`Your ${log.code('meta')} object is missing the following requires properties:\n` + missingMeta);
export default async (options) => {
    const startTime = performance.now();
    const isTheme = options.mode === 'dist' || options.mode === 'dev' || false;
    const fileName = options.mode !== 'addon'
    ...
    if (!fs.existsSync(options.target))
        log.error(`Cannot find the target file ${log.code(options.target)}`);
